...
for rel,sha in scope_boundary['sources'].items():
    ...
    with (ROOT.parent/rel).open('rb') as handle:
        for block in iter(lambda:handle.read(1024*1024),b''):digest.update(block)
    assert digest.hexdigest()==sha,rel
for rel,sha in scope_boundary['tables'].items():
    assert hashlib.sha256((ROOT/rel).read_bytes()).hexdigest()==sha,rel
bound_tables|={ROOT/rel for rel in scope_boundary['tables']}
scope_dir=ROOT.parent/'outputs/paper_a_safe_correction/scope_boundary_v1'
scope_boundary_audit=json.loads((scope_dir/'audit.json').read_text())
assert scope_boundary_audit['status']=='scope_boundary_checks_passed' and not scope_boundary_audit['failures']
assert scope_boundary_audit['observations']==474732 and scope_boundary_audit['bootstrap_replicates']==10000
assert scope_boundary_audit['band_mrr']==.005 and scope_boundary_audit['min_training_triples']==50
assert not any(scope_boundary_audit[k] for k in ('selector_fits','scorer_runs','new_policy_selection','test_used_for_selection',
    'historical_results_replaced','global_necessity_established','strata_maps_refitted_in_bootstrap'))
assert all(c['original_replay_passed'] and c['prior_outcomes_and_gate_effects_reconciled']
    and c['prior_oracle_and_fallback_reconciled'] and c['four_stages_disjoint_exhaustive'] for c in scope_boundary_audit['checks'])
scope_records={}
for name,count in [('summary',12),('seed_direction',72),('dev_strata',24),('dev_strata_seed_direction',144),
    ('dev_training_cells',scope_boundary_audit['training_cell_rows']),('original_settings',36)]:
    with (scope_dir/(name+'.csv')).open(encoding='utf-8') as handle:scope_records[name]=list(csv.DictReader(handle))
    assert len(scope_records[name])==count
for r in scope_records['summary']+scope_records['seed_direction']+scope_records['dev_strata']:
    assert sum(int(r[k]) for k in ('rejected_active_n','rejected_idle_n','accepted_idle_n','active_n'))==int(r['n'])
    assert int(r['rejected_n'])==int(r['rejected_active_n'])+int(r['rejected_idle_n'])
    assert int(r['harm_n'])+int(r['benefit_n'])+int(r['unchanged_n'])==int(r['n'])
    if int(r['n']):assert abs(float(r['mean_gain'])-float(r['mean_loss'])-float(r['utility']))<1e-12
assert all(int(r['valid_replicates'])==10000 for r in scope_records['dev_strata'] if int(r['n']))
for label in ('W-NA','D-NA'):
    r=next(r for r in scope_records['summary'] if r['label']==label and r['split']=='test')
    assert float(r['utility_lo'])<0<float(r['utility_hi'])
    assert float(r['matched_gate_lo'])<0<float(r['matched_gate_hi'])
    assert float(r['raw_utility'])<0 and float(r['gate_lo'])>0 and float(r['oracle_gap'])>0
assert r'\label{app:scope-boundary}' in main
assert 'scope condition, not an established necessary condition' in main
assert 'No globally ineligible pair is evaluated' in main
assert '96.39\\%' in main and '95.27\\%' in main
...
for rel,sha in coverage_quality['sources'].items():
    ...
    assert digest.hexdigest()==sha,rel
for collection in ('tables','figures'):
    for rel,sha in coverage_quality[collection].items():
        assert hashlib.sha256((ROOT/rel).read_bytes()).hexdigest()==sha,rel
bound_tables|={ROOT/rel for rel in coverage_quality['tables']}
coverage_dir=ROOT.parent/'outputs/paper_a_safe_correction/coverage_quality_v1'
coverage_audit=json.loads((coverage_dir/'audit.json').read_text())
assert coverage_audit['status']=='coverage_quality_checks_passed' and not coverage_audit['failures']
assert coverage_audit['observations']==474732 and len(coverage_audit['checks'])==12
assert coverage_audit['random_replicates']==512 and coverage_audit['deciles']==list(range(11))
assert coverage_audit['pools']==['All','Active'] and coverage_audit['primary_pair_overlap_verified']
assert coverage_audit['random_conditional_ratios_computed_per_draw']
assert not any(coverage_audit[k] for k in ('selector_fits','scorer_runs','policy_selection','test_used_for_selection',
    'historical_results_replaced','random_bands_are_population_ci','pairs_are_independent'))
assert all(c['gold_invariant_units'] and c['same_query_actions_verified'] and c['nested_harm_verified']
    and c['random_budgets_exact'] and c['zero_and_full_endpoints_verified'] and c['ap_populations_verified']==3
    and c['invalid_rows']==0 for c in coverage_audit['checks'])
coverage_records={}
for name,count in [('curves',528),('aggregates',176),('average_precision',48),('weights',8)]:
    with (coverage_dir/(name+'.csv')).open(encoding='utf-8') as handle:coverage_records[name]=list(csv.DictReader(handle))
    assert len(coverage_records[name])==count
for r in coverage_records['curves']:
    if r['method']=='Confidence':
        assert float(r['harm_n'])<=float(r['active_n'])<=float(r['accepted_n'])<=float(r['n'])
        assert abs(float(r['harm_all'])-float(r['harm_n'])/float(r['n']))<1e-12
    if r['pool']=='Active':assert abs(float(r['active_n'])-float(r['accepted_n']))<1e-12
for r in coverage_records['average_precision']:
    assert abs(float(r['ap'])-float(r['prevalence'])-float(r['ap_lift']))<1e-12
    if r['scope']=='pair':assert abs(float(r['sklearn_error']))<1e-12
for split in ('test','dev_oof'):
    records=[r for r in coverage_records['weights'] if r['split']==split]
    assert abs(sum(float(r['micro_full_weight']) for r in records)-1)<1e-12
    assert all(float(r['macro_weight'])==.25 and float(r['same_dataset_pair_observation_overlap'])==1 for r in records)
assert r'\label{app:coverage-quality}' in main and r'\label{eq:coverage-ap}' in main
assert 'mechanically nondecreasing' in main and '69.85' in main and 'not trapezoidal' in main
...
for rel,sha in alignment_training['sources'].items():
    ...
    with (ROOT.parent/rel).open('rb') as handle:
        for block in iter(lambda:handle.read(1024*1024),b''): digest.update(block)
    assert digest.hexdigest()==sha,rel
for rel,sha in alignment_training['tables'].items():
    assert hashlib.sha256((ROOT/rel).read_bytes()).hexdigest()==sha,rel
bound_tables|={ROOT/rel for rel in alignment_training['tables']}
alignment_dir=ROOT.parent/'outputs/paper_a_safe_correction/alignment_training_v1'
alignment_audit=json.loads((alignment_dir/'audit.json').read_text())
assert alignment_audit['status']=='alignment_training_checks_passed'
assert alignment_audit['checkpoints']==18 and alignment_audit['mapping_checks']==36
assert alignment_audit['support_checks']==18 and alignment_audit['reciprocal_checkpoint_checks']==6
assert not alignment_audit['historical_execution_commit_established']
assert alignment_audit['training_runs']==alignment_audit['full_candidate_scorer_runs']==0
assert not alignment_audit['historical_results_replaced']
alignment_versions=json.loads((alignment_dir/'source_versions.json').read_text())
assert len(alignment_versions['local_sources'])==66
for entry in alignment_versions['local_sources']:
    raw=(ROOT.parent/entry['path']).read_bytes().replace(b'\r\n',b'\n')
    assert hashlib.sha256(raw).hexdigest()==entry['lf_normalized_sha256'],entry['path']
assert r'\label{app:alignment-training}' in main and 'historical execution revision' in main
