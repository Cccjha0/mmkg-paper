...
Motivated by this bounded-gain structure, we reformulate the problem from always-on fusion to query-level selective activation and propose a lightweight gain-threshold routing framework. The method uses \texttt{Gate-only} as a fusion expert, \texttt{Residual-only} as a structural expert, and a protocol-aware router trained from development-side query-level gain labels. Under the unified query-level routing line, the best learned router (XGBoost with $\delta=0.01$ and $\tau=0.7$) reaches an overall MRR of \texttt{0.3160}, outperforming the fixed \texttt{Residual-only} expert (\texttt{0.2930}), the rule-based router, and the original always-on \texttt{Full Model}, while still leaving room to Oracle routing.
...
\begin{figure}[t]
    ...
    \begin{minipage}[t]{0.29\textwidth}
        \centering
        \textbf{Head-side targets}\\[0.3em]
        {\small Partially image-available}\\[0.6em]
        \fbox{\parbox[c][1.0cm][c]{0.78\linewidth}{\centering \texttt{head\_has\_img}}}\\[0.5em]
        \fbox{\parbox[c][1.0cm][c]{0.78\linewidth}{\centering \texttt{head\_no\_img}}}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.30\textwidth}
        \centering
        \vspace*{0.7em}
        \textbf{Target position is intrinsically entangled}\\
        \textbf{with modality availability}\\[1.1em]
        {\small Bidirectional evaluation}\\
        {\small becomes inherently asymmetric}
    \end{minipage}\hfill
    \begin{minipage}[t]{0.29\textwidth}
        \centering
        \textbf{Tail-side targets}\\[0.3em]
        {\small Fully image-unavailable}\\[0.6em]
        \fbox{\parbox[c][2.7cm][c]{0.78\linewidth}{\centering \texttt{tail\_no\_img}}}
    \end{minipage}
...
\paragraph{Focused three-model subgroup view.}
...
\paragraph{Learned-router subgroup pattern.}
...
Once these pieces are viewed together, the natural conclusion is that the original always-on multimodal setup is mismatched to the protocol. The protocol does not reward multimodal activation uniformly. It rewards it only under specific query conditions. In that sense, bounded multimodal gain is already a routing problem in disguise. The real challenge is not to force one model path to dominate everywhere, but to decide when multimodal evidence is likely to be worth trusting~\cite{zhao2022mose,liang2023hrgat,jacobs1991adaptive,shazeer2017outrageously}.
