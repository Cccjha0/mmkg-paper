...
The second learned router is XGBoost. The current implementation uses \texttt{n\_estimators=300}, \texttt{max\_depth=4}, \texttt{learning\_rate=0.05}, \texttt{subsample=0.8}, \texttt{colsample\_bytree=0.8}, \texttt{reg\_lambda=1.0}, \texttt{objective=binary:logistic}, \texttt{eval\_metric=logloss}, \texttt{random\_state=42}, and \texttt{n\_jobs=1}. Class imbalance is handled through \texttt{scale\_pos\_weight = N\_neg / N\_pos} computed from the current training labels.
