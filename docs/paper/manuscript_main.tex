...
This boundary is not incidental. The paper begins from the asymmetry created by the current protocol, so its contribution should be read as a protocol-grounded demonstration rather than as a universal MMKGC prescription.
...
Accordingly, the finding that multimodal gain is strongest in \\texttt{head\\_has\\_img} should not be read as a universal superiority of head-side prediction, and the dominance of structural fallback on the tail side should not be read as an inherent property of multimodal KGC. A more symmetric benchmark could produce a different local gain distribution.
...
Alternative expert choices, softer routing schemes, end-to-end co-adaptive training, and representation-level routing remain open. The present paper does not attempt to cover that broader design space; it shows only that bounded multimodal gain can already be operationalized effectively with a simple and interpretable routing framework.
...
In particular, some tables require careful reading because the fixed experts in the routing comparison are recomputed on the routing-compatible basis rather than copied from the official main-result line. This is the correct choice for internal consistency, but it adds interpretive cost.

\\subsection{Coarseness of Relation-Group and Feature Design}
The relation-group analysis is deliberately coarse and analysis-oriented. It is useful for testing broad tendencies, but it does not provide a complete semantic ontology of multimodal-favorable relation types.

The router feature design is similarly coarse-but-sufficient. It is enough to support the current controlled claim, but it is not intended as a complete characterization of all useful routing signals.

For this reason, the paper should not be read as offering a final taxonomy of multimodal-favorable relations or an exhaustive feature blueprint. It provides only a first operational decomposition showing that target regime, relation priors, and expert-confidence signals are already sufficient to support selective activation under the present protocol.
...
The paper therefore does not deliver a universally dominant multimodal architecture. What it does deliver instead is a protocol-aware result: once a bounded-gain regime is identified clearly, selective activation can use multimodal evidence more effectively than always-on multimodal use under the present setting.
