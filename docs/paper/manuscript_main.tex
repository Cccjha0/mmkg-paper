...
Relation-group evidence therefore remains auditable as supporting analysis: the grouped ordering is reported together with the retained minimum-support rule and the retained support for each coarse relation group. Across all three retained coarse relation groups, the ordering remains \texttt{Residual-only > Full Model > Gate-only}. This indicates that relation dependence is real, but still bounded: relation conditions affect where multimodal benefit is more plausible, yet they do not overturn the broader structure-dominant pattern established by the official main results. It shows that relation dependence is real, but still bounded.
