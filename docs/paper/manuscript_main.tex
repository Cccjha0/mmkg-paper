...
Knowledge graph completion (KGC) aims to infer missing facts from observed triples and remains a central task for improving knowledge graph quality~\cite{trouillon2016complex,dettmers2018conve,sun2019rotate}. In multimodal knowledge graph completion (MMKGC), entities may additionally be associated with auxiliary modalities such as text and images, making it possible to enrich entity representations beyond pure graph structure~\cite{liu2019mmkg,xie2017ikrl,mousselly2018multimodal,pezeshkpour2018mkbe,ngiam2011multimodal,baltrusaitis2019multimodal}. In product-oriented knowledge graphs, this possibility is especially appealing because many relations appear intuitively compatible with visual evidence. A natural expectation is therefore that once text and image signals are incorporated into link prediction, multimodal models should outperform structure-only alternatives.
...
Another closely related research direction concerns robustness under incomplete or unreliable modalities. In many real-world settings, entities do not have uniformly available text or images, and even when multimodal features exist, their quality may vary substantially. This has motivated work on missing-modality learning, incomplete multimodal representation learning, and robust fusion under noisy inputs. The broader lesson from this literature is that multimodal systems should not assume perfectly balanced or fully available auxiliary information~\cite{baltrusaitis2019multimodal,wang2021rsme,zhao2022mose,jian2025amit}.
...
Classical structural KGC models remain an important reference point for any MMKGC study. Even when multimodal information is available, strong structural baselines can remain highly competitive, especially if the graph already contains regularities that are easy to exploit. This is one reason that multimodal gains should not be evaluated only relative to weak baselines; they must also be judged against structure-heavy alternatives that may still dominate overall~\cite{trouillon2016complex,balazevic2019tucker,dettmers2018conve,sun2019rotate}.

This issue is central to our setting. Under the current OpenBG-IMG protocol, \texttt{ComplEx}~\cite{trouillon2016complex} emerges as a competitive structural baseline, while \texttt{Residual-only} is the strongest overall model in our internal family. By contrast, \texttt{TuckER}~\cite{balazevic2019tucker}, although a classical structural reference model, does not emerge as a competitive structural baseline under the current split and protocol. This distinction matters for interpretation. The main empirical tension in our paper is not between \texttt{Full Model} and a weak reference system, but between local multimodal gains and globally stronger structure-dominant alternatives.
...
\begin{figure}[t]
    ...
    \caption{Role--modality asymmetry in OpenBG-IMG. Tail entities are image-unavailable, whereas head entities are only partially image-available. As a result, target position is intrinsically entangled with modality availability in bidirectional link prediction, yielding three meaningful target-side regimes: \texttt{head\_has\_img}, \texttt{head\_no\_img}, and \texttt{tail\_no\_img}.}

    \label{fig:protocol_asymmetry}

\end{figure}

Figure~\ref{fig:protocol_asymmetry} makes this role--modality asymmetry explicit. The subgroup design used later in the paper is therefore not an artificial decomposition introduced for convenience; instead, it makes the dataset's intrinsic target-side modality regimes explicit. This is also why later subgroup results should not be interpreted as fully symmetric evidence about multimodal usefulness. Under the current setting, the question is not whether multimodal information helps equally in all directions, but where it remains available and informative enough to matter.
...
All formal comparisons use filtered ranking metrics on the test set, including MRR, Hits@1, Hits@3, and Hits@10.
...
The relation-type analysis is conducted under the same protocol rather than under a separately constructed benchmark. We use a coarse grouping file that organizes relations into \texttt{visual\_relations}, \texttt{weak\_visual\_relations}, and \texttt{ambiguous\_material\_relations}.
...
The protocol described above is not a neutral implementation detail. It is the foundation of both the paper's analysis and its method contribution. Under a more symmetric split or a differently balanced modality distribution, the relative strength of multimodal fusion, structural compensation, and routing could differ. In the current setting, however, the combination of target-position asymmetry, image-availability asymmetry, relation dependence, and branch competition produces a specific empirical pattern: multimodal gain is visible but local, stronger structural compensation remains globally dominant, and multimodal usefulness is therefore query-dependent rather than globally reliable.
...
For a query \(q\) and a candidate entity \(e\), let \(s_f(q,e)\) denote the score from the fusion expert and \(s_s(q,e)\) denote the score from the structural expert.
...
This design has two advantages. First, it keeps the experts fixed and changes only the selection mechanism, so any improvement can be attributed to selective activation rather than to added model capacity or end-to-end co-adaptation. Second, it matches the current problem structure well: the goal is not to blend experts softly everywhere, but to activate the multimodal path only when it is sufficiently likely to help~\cite{jacobs1991adaptive,jordan1994hierarchical,shazeer2017outrageously}.

\subsection{Gain Label Construction}
To train the router, we construct query-level supervision from development-set expert performance. For each query \(q\), let \(\mathrm{rank}_f(q)\) and \(\mathrm{rank}_s(q)\) denote the filtered ranks of the correct target entity under the fusion expert and the structural expert, respectively.
...
The second feature group describes whether the query appears to have coherent multimodal evidence. These features include \texttt{text\_img\_cosine} and \texttt{img\_is\_missing\_replaced}.
...
For the learned routers, we evaluate multiple gain margins \(\delta\) and multiple routing thresholds \(\tau\). This separation is important because \(\delta\) controls how gain-positive labels are defined during training, whereas \(\tau\) controls how conservatively the router activates fusion at inference time.
