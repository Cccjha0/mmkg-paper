...
Although the routing table shows overall improvement, overall metrics alone do not reveal whether the learned router behaves consistently with the protocol-aware gain structure that motivated the method. We therefore evaluate all routing models over the same three meaningful target-side regimes: \texttt{head\_has\_img}, \texttt{head\_no\_img}, and \texttt{tail\_no\_img}.
...
The learned routers follow the expected selective pattern. As the threshold \(\tau\) increases, fusion coverage declines across all regimes, but the decline is especially consequential in \texttt{tail\_no\_img}, where stronger structural fallback is more reliable. For the strongest learned setting (\texttt{xgb}, \texttt{delta=0.01}, \texttt{tau=0.7}), the subgroup MRR values are \texttt{0.01161} on \texttt{head\_has\_img}, \texttt{0.01253} on \texttt{head\_no\_img}, and \texttt{0.62003} on \texttt{tail\_no\_img}.

The corresponding subgroup fusion coverage values are \texttt{0.1501}, \texttt{0.1059}, and \texttt{0.2326} for \texttt{head\_has\_img}, \texttt{head\_no\_img}, and \texttt{tail\_no\_img}, respectively.
