...
We investigate this problem on OpenBG-IMG under a unified protocol with filtered ranking, \texttt{direction=both}, and three-seed reporting. First, we conduct a protocol-aware analysis and show that the current split exhibits clear role--modality asymmetry: head-side targets may be image-available, whereas tail-side targets are effectively image-unavailable. As a result, multimodal gain is strongest in image-supported head-side regimes but remains bounded overall, while the strongest fixed model is still the structure-heavy \texttt{Residual-only} model.

Then, we propose a lightweight gain-threshold routing framework for selective multimodal activation. The framework uses \texttt{Gate-only} as a fusion expert and \texttt{Residual-only} as a structural expert, constructs development-side query-level gain labels from reciprocal-rank improvement, and trains a protocol-aware router with target-regime, relation-prior, modality-consistency, and expert-confidence features. Finally, a hard threshold is used to decide whether the current query should activate the multimodal path or fall back to the structural expert.

Finally, the proposed method achieves better performance than fixed-expert and always-on multimodal baselines under the unified query-level routing line. The best learned router (XGBoost with $\delta=0.01$ and $\tau=0.7$) reaches an MRR of \texttt{0.3160}, outperforming \texttt{Residual-only} (\texttt{0.2930}), rule-based routing (\texttt{0.2939}), and the original \texttt{Full Model} (\texttt{0.2100}), while Oracle routing (\texttt{0.3337}) still indicates additional headroom. These results show that the contribution of this work lies not in a universally dominant multimodal architecture, but in a protocol-aware transition from bounded-gain analysis to gain-aware selective activation.
\end{abstract}
...
\begin{enumerate}[leftmargin=1.5em]
    \item We reveal a \textbf{protocol-aware bounded-gain mechanism} on OpenBG-IMG. Specifically, we show that role--modality asymmetry entangles target position with image availability, causing multimodal benefit to be local rather than globally uniform.
