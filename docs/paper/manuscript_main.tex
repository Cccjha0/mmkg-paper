...
\begin{figure}[t]
    \centering
    \IfFileExists{Figure1.pdf}
      {\includegraphics[width=0.8\textwidth]{Figure1.pdf}}
      {\fbox{\parbox{0.75\textwidth}{\centering Figure~1 placeholder: protocol-defined target-side availability structure in OpenBG-IMG.}}}
...
The official line compares the seven completed paper-stage models: \texttt{Text-only}, \texttt{Early Fusion}, \texttt{Gate-only}, \texttt{Residual-only}, \texttt{Full Model}, \texttt{ComplEx}, and \texttt{TuckER}.

\paragraph{Routing line.}
The routing line compares \texttt{Gate-only} (fixed fusion expert), \texttt{Residual-only} (fixed structural expert), rule-based routing, logistic routing, XGBoost routing, and Oracle routing on a shared query-level recomputed basis.
...
The official line therefore establishes the paper's central tension: the strongest fixed model is structure-heavy, yet multimodal modeling still matters within the multimodal family because \texttt{Full Model} consistently outperforms \texttt{Gate-only}, \texttt{Early Fusion}, and \texttt{Text-only}.
...
Within this line, the strongest learned router is XGBoost with $\delta=0.01$ and $\tau=0.7$, reaching \texttt{0.3160} MRR and improving over both the fixed \texttt{Residual-only} expert and the rule-based router. Oracle routing indicates that additional routing headroom still remains.
...
This paper necessarily relies on two complementary evaluation dialects: the official model-comparison line and the routing-compatible recomputed line. That separation is methodologically necessary for fairness, but it also makes the paper more difficult to present and interpret than if every number came from one single aggregation pipeline.
