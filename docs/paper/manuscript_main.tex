...
We keep a focused three-model comparison among \texttt{Gate-only}, \texttt{Full Model}, and \texttt{Residual-only}. Across all three coarse relation groups, the ordering remains stable: \texttt{Residual-only} performs best, \texttt{Full Model} ranks second, and \texttt{Gate-only} remains third.

\begin{table}[t]
    \centering
    \caption{Relation-group MRR for the focused three-model comparison.}
    \label{tab:relation_group_results}
    \begin{tabular}{lccc}
        \toprule
        Group & Gate-only & Full Model & Residual-only \\
        \midrule
        \texttt{visual\_relations} & 0.1350 & 0.1826 & 0.2691 \\
        \texttt{weak\_visual\_relations} & 0.2194 & 0.2252 & 0.3197 \\
        \texttt{ambiguous\_material\_relations} & 0.1337 & 0.1948 & 0.2842 \\
        \bottomrule
    \end{tabular}
\end{table}
