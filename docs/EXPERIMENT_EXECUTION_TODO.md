...
- 当前更适合强结构表示的典型场景可概括为：
  - 预测目标位于 `tail` 侧
  - 目标实体无图
  - 关系更接近场景、材质、规格、净含量等结构或元数据约束
  - 代表性正文案例包括：
    - `适用场景`
    - `材质`
    - `净含量`
- 因此 `8.3` 的正式收口结论是：
  - 案例层证据与 `6.1`、`6.2`、`6.3`、`7.2` 的统计和行为分析一致
  - 多模态最适合“head + has_img + 局部可感知属性”的局部有利场景
  - 强结构表示最适合“tail + no_img + 结构可预测尾实体”的全局主导场景
  - 案例分析不是孤立补充，而是对“收益边界”主线的样本级验证
...
- [x] 当前项目已满足 `9.2` 的判定条件
- 已新增最终主线定稿文档：
  - `docs/FINAL_NARRATIVE_DECISION.md`
- 当前证据链已经完整覆盖：
  - 主结果：`Residual-only > Full Model > Gate-only`
  - `6.1 has_img / no_img`：最强局部收益出现在 `head_has_img`
  - `6.2 relation type`：`Full Model` 在多数中等支持度关系上优于 `Gate-only`，但很少超过 `Residual-only`
  - `7` 行为分析：fusion 有真实贡献，但最终长期表现为 `residual-dominant asymmetric complementarity`
  - `8` 案例分析：成功案例集中于 `head + has_img`，失败案例集中于 `tail + no_img`
- 因此论文主线正式定稿为：
  - 不是“Full Model 全局优于结构基线”
  - 而是“OpenBG-IMG 上多模态收益何时有效，以及为什么它具有明确边界”
- `9.1` 不再作为当前稿件主线
- `9.3` 也不作为当前稿件主线，因为当前证据已经表明多模态局部收益是真实存在的，而不是几乎完全缺失
