...
- `Full Model` 目前没有缩小与强结构基线的差距，反而仍明显落后

原因与依据：

- `ComplEx / TuckER` 已经在当前统一协议下补跑
- 当前正式结论已不再局限于五组主模型内部比较
- 如果 `Full Model` 仍低于 `ComplEx / TuckER`，则不能将论文主线收紧为“方法已接近强结构最优”

这说明当前更合理的工作方向是：

- 接受强结构基线仍占优这一事实
- 将重点转向“为什么多模态收益没有跨过强结构表示”这一分析问题
...
5. 下一步最关键的工作不是继续补主结果，而是：
   - 进入 residual dominance 分析
   - 进入 `has_img / no_img` 与关系类型分组分析
