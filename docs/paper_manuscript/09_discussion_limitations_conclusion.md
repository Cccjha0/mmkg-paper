...
What the current study shows is therefore not that multimodal information fails, but that its contribution is bounded by protocol conditions. This is a more informative conclusion than either naive optimism ("multimodal models should always win") or naive pessimism ("multimodal information is useless here"). The evidence supports a middle position: multimodal gain is real, but it appears in a favorable local regime rather than uniformly across the task.
